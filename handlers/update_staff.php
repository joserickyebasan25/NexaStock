...
try {
    ...
    $saved = $users->updateStaff($id, $fname, $lname, $email, $role, $password ?: null, $photo);

    if ($saved && $photo && !empty($existing['photo'])) {
        deleteLocalPhoto($existing['photo']);
    }

    echo json_encode([
        'success' => $saved,
        'message' => $saved ? 'Staff updated successfully.' : 'Could not update staff.'
    ]);
} catch (Exception $e) {
